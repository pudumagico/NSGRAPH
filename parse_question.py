        ...
        for data in docs:
            if data['graph']['id'] == graph_id:
                ...
                for i, regex in enumerate(question_forms):
                    ...
                    if match:
                        args = map(str.lower, list(match.groups()))
...
question_forms = [
    ...
    re.compile(
        "What is the cleanliness level of ([a-zA-Z]+) station\?"),
    # (lambda s: Pick(s, "cleanliness")),
    ...
    re.compile(
        "Describe ([a-zA-Z]+) station's architectural style."),
    # (lambda s: Pick(s, "architecture")),
    ...
    re.compile(
        "How many music styles does ([a-zA-Z]+) pass through\?"),
    # (lambda l: Count(Unique(Pluck(Nodes(Filter(AllEdges(), "line_id", Pick(l, "id"))),
    # 						  "music"))) ),
    # "LineTotalMusicCount"),
    re.compile(
        "How many sizes of station does ([a-zA-Z]+) pass through\?"),
    # (lambda l: Count(Unique(Pluck(Nodes(Filter(AllEdges(), "line_id", Pick(l, "id"))),
    #   "size"))) ),
    re.compile(
    ...
    re.compile(
        "Which ([a-zA-Z]+) station is adjacent to ([a-zA-Z]+)\?"),
